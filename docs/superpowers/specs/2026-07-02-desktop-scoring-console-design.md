...
- **Mobile-first** : de par son statut d'outil de secours, la console (écran de sélection, formulaires « Nouveau match »/« score de départ », et écran de saisie 3 colonnes) doit rester pleinement utilisable depuis un navigateur de téléphone — c'est justement l'appareil qu'on aurait sous la main si l'app plante. Le tableau de bord déjà livré n'est pas concerné (moins critique en mode dégradé, non retouché dans ce sous-projet). Sur petit écran, les 3 colonnes de la console s'empilent verticalement dans le même ordre de lecture qu'en desktop : ScoreCard + « Annuler » en premier, puis la grille de 8 boutons + stats live, puis le déroulé point par point en dernier (accessible en scrollant, pas de masquage conditionnel). Cibles tactiles ≥ 48px comme partout ailleurs dans le design system.
...
**Console** (`app/dashboard/console/[sessionId]/page.tsx`, CSS Grid 3 colonnes conforme au README #9 au-delà d'un breakpoint desktop, colonne unique empilée en dessous — approche mobile-first : les styles de base (sans media query) sont ceux de l'empilement vertical, une media query `min-width` élargit vers la grille 3 colonnes) :
- **Gauche (340px en desktop, pleine largeur en mobile, affiché en premier)** : `ScoreCard` (réutilise la logique d'affichage de `ScoreTable.tsx`/`SetTrail.tsx` de la page publique, adaptée au thème clair/sombre du dashboard) + bouton « Annuler le dernier point » (hauteur ≥ 48px).
- **Centre (affiché en second)** : grille des 8 boutons contextuels (4 « mon point » mis en avant visuellement, 4 « point adverse », chacun ≥ 48px de haut, empilés 1 colonne en mobile / 2 colonnes en desktop) + tuiles de stats live dérivées d'un simple comptage des `context` déjà enregistrés pour la session (nombre d'aces, de coups gagnants, de fautes, etc. — calcul client, pas de nouvel endpoint d'agrégation).
- **Droite (288px en desktop, pleine largeur en mobile, affiché en dernier)** : déroulé point par point (liste scrollable), alimentée par `GET /sessions/{id}/points`.
- **Écran de sélection et formulaires** (« Nouveau match », « score de départ ») : champs empilés verticalement en une colonne à toutes les tailles d'écran (pas de mise en page multi-colonnes à adapter), boutons/champs ≥ 48px — déjà mobile-friendly par construction, pas d'effort de layout supplémentaire au-delà des tailles tactiles.
...
- **Responsive** : vérification manuelle (pas de test automatisé pertinent pour du CSS) de l'écran de sélection, des formulaires et de la console sur un viewport mobile (≤ 480px) et un viewport desktop, avant de considérer le sous-projet terminé.
