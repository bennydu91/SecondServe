...
- [x] [Review][Defer] `created_at` client non validé côté serveur — deferred, app mono-utilisateur
- [x] [Review][Defer] `SessionsResponse` défini mais jamais utilisé (GET /sessions absent) — deferred, sera utilisé dans une story future [`backend/app/features/sessions/schemas.py`]
